...
export default function FleetApp() {
  ...
  const [newResource, setNewResource] = useState({
    name: '', plate: '', type: '', capacity: 'Medium', distance: '', estTime: '', license: ''
  });
  ...
  useEffect(() => {
    ...
    // If we found old assignments, update state
    if (archivedAssignments.length > 0) {
        setAssignments(keptAssignments);
        setHistory(prev => [...archivedAssignments, ...prev]); 
        
        setDrivers(prevDrivers => prevDrivers.map(d => 
            resourcesToFree.drivers.includes(d.id) ? { ...d, status: 'available' } : d
        ));
        
        setTrucks(prevTrucks => prevTrucks.map(t => 
            resourcesToFree.trucks.includes(t.id) ? { ...t, status: 'available' } : t
        ));
    } else {
  ...
  const handleSaveResource = () => {
    if (editingId) {
      ...
      if (addModalType === 'truck') {
         ...
         if (!newResource.name) return;
         // Regenerate initials if name changed, or keep old? Let's regen.
    ...
    return (
      <div className={`bg-white rounded-xl shadow-sm border-l-4 p-4 mb-4 relative overflow-hidden ${isHistory ? 'border-slate-400' : 'border-blue-500'}`}>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
           ...
           {!isHistory && (
               <button 
                 onClick={() => handleCompleteShift(data.id)}
                 className="text-green-600 hover:text-green-700 text-sm font-semibold flex items-center gap-1 active:scale-95 transition-transform"
               >
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20 font-sans">
        ...
        {activeTab === 'records' && (
            <div className="space-y-6 animate-fade-in">
                <div className="flex items-center justify-between">
                    ...
                    <button 
                        onClick={exportToCSV}
                        className="flex items-center gap-2 bg-slate-800 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-md active:scale-95 transition-transform"
                    >
          ...
          <div className="space-y-6 animate-fade-in">
             ...
             <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
               <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 font-semibold text-sm text-slate-600 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span>Operators</span>
                    <span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full">{drivers.length}</span>
                  </div>
                  <button onClick={() => openAddModal('driver')} className="text-blue-600 hover:bg-blue-50 p-1 rounded transition-colors">
                    <Plus size={18} />
                  </button>
               </div>
               <div className="max-h-[300px] overflow-y-auto">
                {drivers.map((d, i) => (
                  <div key={d.id} className={`flex items-center justify-between p-4 ${i !== drivers.length -1 ? 'border-b border-slate-100': ''}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <button 
                          onClick={() => openEditModal('driver', d)}
               ...
               {drivers.length === 0 && <div className="p-4 text-center text-slate-400 text-sm">No operators added</div>}
             </div>
             ...
             <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
               <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 font-semibold text-sm text-slate-600 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span>Vehicles</span>
                    <span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full">{trucks.length}</span>
                  </div>
                  <button onClick={() => openAddModal('truck')} className="text-blue-600 hover:bg-blue-50 p-1 rounded transition-colors">
                    <Plus size={18} />
                  </button>
               </div>
               <div className="max-h-[300px] overflow-y-auto">
                 {trucks.map((t, i) => (
                   <div key={t.id} className={`flex items-center justify-between p-4 ${i !== trucks.length -1 ? 'border-b border-slate-100': ''}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <button 
                          onClick={() => openEditModal('truck', t)}
                          className="text-slate-400 hover:text-blue-500 transition-colors p-2"
                        >
                          <Edit2 size={16} />
                        </button>
                        <button 
                          onClick={() => handleRemoveTruck(t.id)}
                          className="text-slate-300 hover:text-red-500 transition-colors p-2"
                        >
                          <Trash2 size={16} />
                        </button>
               ...
               {trucks.length === 0 && <div className="p-4 text-center text-slate-400 text-sm">No vehicles added</div>}
             </div>
             ...
             <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
               <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 font-semibold text-sm text-slate-600 flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span>Routes</span>
                    <span className="text-xs bg-slate-200 px-2 py-0.5 rounded-full">{routes.length}</span>
                  </div>
                  <button onClick={() => openAddModal('route')} className="text-blue-600 hover:bg-blue-50 p-1 rounded transition-colors">
                    <Plus size={18} />
                  </button>
               </div>
               {routes.map((r, i) => (
                 <div key={r.id} className={`flex items-center justify-between p-4 ${i !== routes.length -1 ? 'border-b border-slate-100': ''}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="overflow-hidden">
                        <p className="text-sm font-medium text-slate-800 truncate w-40 sm:w-60">{r.name}</p>
                        <p className="text-xs text-slate-500">{r.distance} • {r.estTime}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button 
               ...
               {routes.length === 0 && <div className="p-4 text-center text-slate-400 text-sm">No routes added</div>}
             </div>
          </div>
        )}
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">Enable system notifications.</p>
              </div>
              <button 
                onClick={requestNotificationPermission}
                className={`p-3 rounded-full transition-colors ${notificationsEnabled ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}
              >
                <Bell size={24} />
              </button>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                ...
                <p className="text-sm text-slate-500">
                    Dispatched trucks are automatically moved to the "Records" tab 24 hours after they are started.
                </p>
            </div>
             ...
             <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 mt-4">
                ...
                <button 
                  ...
                  className="w-full text-red-600 border border-red-200 bg-red-50 hover:bg-red-100 font-semibold py-2 rounded-lg transition-colors"
                >
                    Reset Application
                </button>
             </div>
      ...
      {/* Floating Action Button (FAB) - Only show on dashboard for dispatch */}
      {activeTab === 'dashboard' && (
        <button 
          onClick={() => setShowDispatchModal(true)}
          className="fixed bottom-24 right-6 bg-blue-600 text-white p-4 rounded-full shadow-lg shadow-blue-600/30 hover:bg-blue-700 active:scale-95 transition-all z-40"
        >
          <Plus size={24} />
        </button>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-sm p-0 sm:p-4 animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-lg text-slate-800">Dispatch Vehicle</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {routes.length === 0 ? (
                    <div className="p-4 text-center border border-dashed rounded-xl text-slate-400 text-sm">
                      No routes available. Go to Fleet tab to add.
                    </div>
                  ) : (
                    routes.map(route => (
                      <button
                        key={route.id}
                        onClick={() => setNewAssignment({...newAssignment, routeId: route.id})}
                        className={`p-3 rounded-xl border text-left transition-all ${
                          newAssignment.routeId === route.id 
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  onChange={(e) => setNewAssignment({...newAssignment, driverId: e.target.value})}
                >
                ...
                {drivers.filter(d => d.status === 'available').length === 0 && (
                   <p className="text-xs text-amber-600 flex items-center gap-1"><AlertCircle size={12}/> No drivers available</p>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  onChange={(e) => setNewAssignment({...newAssignment, truckId: e.target.value})}
                >
              ...
              <div className="space-y-2">
                 ...
                 <div className="flex gap-2 overflow-x-auto pb-2">
                   {SHIFTS.map(shift => (
                     <button
                       key={shift}
                       onClick={() => setNewAssignment({...newAssignment, shift})}
                       className={`whitespace-nowrap px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
              <button 
                ...
                disabled={!newAssignment.driverId || !newAssignment.truckId || !newAssignment.routeId}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-600/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
              >
                Confirm Dispatch
              </button>
            </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h3 className="font-bold text-lg text-slate-800">
                {editingId ? 'Edit' : 'Add'} {addModalType === 'truck' ? 'Truck' : addModalType === 'route' ? 'Route' : 'Operator'}
              </h3>
            ...
            <div className="p-6 space-y-4">
              {addModalType === 'truck' && (
                <>
                  <div>
                    ...
                    <input 
                      type="text" 
                      placeholder="e.g. FLT-999"
                      className="w-full mt-1 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                      value={newResource.plate}
                      onChange={e => setNewResource({...newResource, plate: e.target.value})}
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <select 
                      className="w-full mt-1 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none bg-white"
              ...
              {addModalType === 'route' && (
                <>
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Route Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Distance</label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Est. Time</label>
                      <input 
                        type="text" 
                        placeholder="e.g. 1h 30m"
                        className="w-full mt-1 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                        value={newResource.estTime}
                        onChange={e => setNewResource({...newResource, estTime: e.target.value})}
                      />
                    </div>
                  </div>
                </>
              ...
              {addModalType === 'driver' && (
                <>
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">License Type</label>
                    <input 
                      type="text" 
                      placeholder="e.g. CDL-A"
                      className="w-full mt-1 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                      value={newResource.license}
                      onChange={e => setNewResource({...newResource, license: e.target.value})}
                    />
                  </div>
                </>
              )}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100">
              ...
              >
                {editingId ? 'Update' : 'Save'} {addModalType === 'truck' ? 'Truck' : addModalType === 'route' ? 'Route' : 'Operator'}
              </button>
            </div>
          </div>
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-6 py-2 flex justify-between items-center z-40 pb-safe">
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-lg transition-colors ${activeTab === 'records' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center p-2 rounded-lg transition-colors ${activeTab === 'fleet' ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
        >
